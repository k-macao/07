...
默认每个**工作日 18:00（北京时间）**自动执行，也可手动触发。**非交易日（周末/节假日）默认自动回退到最近一个交易日**，按最近交易日行情继续分析并推送；如需恢复"非交易日跳过"的原行为，设置环境变量 `USE_LATEST_TRADING_DAY=false`（或 `TRADING_DAY_CHECK_ENABLED=false`）。强制运行、交易日检查、断点续传等规则见 [完整指南](docs/full-guide.md#定时任务配置)。
