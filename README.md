...
默认每个**工作日 17:00（北京时间，下午五点）**自动执行，也可手动触发；分析完成后**只推送 PushPlus「电子杂志 × 电子墨水」微信杂志长页这一个页面**，不再走企业微信/飞书/Telegram/邮件等其他渠道。**非交易日（周末/节假日）默认自动回退到最近一个交易日**，按最近交易日行情继续分析并推送；如需恢复"非交易日跳过"的原行为，设置环境变量 `USE_LATEST_TRADING_DAY=false`（或 `TRADING_DAY_CHECK_ENABLED=false`）。强制运行、交易日检查、断点续传等规则见 [完整指南](docs/full-guide.md#定时任务配置)。
